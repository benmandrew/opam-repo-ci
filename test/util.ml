...
module Cmd = struct
  ...
  let exec_or_fail_stdout ?cwd cmd =
    let cmd = Array.of_list cmd in
    let cwd = Option.map Fpath.to_string cwd in
    Lwt_process.pread ?cwd ("", cmd)
...
let init cwd =
  let cap_secrets = Result.get_ok @@ Fpath.of_string Opam_repo_ci.Conf.Capnp.cap_secrets in
  Cmd.mkdir ~cwd cap_secrets
  >>= fun () ->
  Cmd.cp [ "-R" ] (Fpath.v "./patches/.") cwd >>= fun () ->
