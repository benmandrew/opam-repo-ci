...
let test_simple () =
  ...
  Current.Db.exec_literal db
    "INSERT INTO cache (op, key, job_id, value, ok, outcome, ready, running, \
     finished, build)\n\
    \                                     VALUES ('test', x'00', 'job1', \
     x'01', 1, x'02', '2019-11-01 9:00', '2019-11-01 9:01', '2019-11-01 9:02', \
     0)";
  Index.set_active_refs ~repo [ ("master", hash) ];
  Index.set_status ~owner ~name ~hash (`Pending, 0);
  Index.record ~repo ~hash
  @@ Index.Job_map.of_list [ ("analysis", Some "job1"); ("alpine", None) ];
  Alcotest.(check (list string)) "Repos" [ "name" ] @@ Index.list_repos owner;
  Alcotest.(check (list (pair string string))) "Refs" [ ("master", hash) ]
  ...
  Current.Db.exec_literal db
    "INSERT INTO cache (op, key, job_id, value, ok, outcome, ready, running, \
     finished, build)\n\
    \                                     VALUES ('test', x'01', 'job2', \
     x'01', 0, x'21', '2019-11-01 9:03', '2019-11-01 9:04', '2019-11-01 9:05', \
     0)";
  Index.set_status ~owner ~name ~hash (`Failed, 0);
  Index.record ~repo ~hash
  @@ Index.Job_map.of_list
  ...
  Index.record ~repo ~hash
  @@ Index.Job_map.of_list [ ("analysis", Some "job1") ];
