...
type 'a f = {
  f : 'x. label:string Current.t -> kind -> 'x Current.t -> 'a Current.t;
}
(** A mapping function for the leaves of the tree: It's quantified over its
    input ['x Current.t] as the actions can produce anything, so [f] will only
    be able to extract their metadata with [job_id] and/or [Current.state]. *)

val flatten :
  'a t -> map:'a f -> merge:('a -> 'a -> 'a) -> empty:'a -> 'a Current.t
(** [flatten t ~map ~merge ~empty] applies [map] to every [action] leaves of the
    tree, then fold all the results with the [merge] function (which should be
    associative, commutative, with [empty] as its zero). *)
...
(** [list_map ord ?collapse_key f lst] is the dynamic list
    [Current.list_map ord ?collapse_key f lst], except that its output is fixed
    to [empty] until the input [lst] is successful. You must ensure that the
    status of the input [lst] is reported elsewhere. *)
...
(** [job_id t] is the job id associated with the ocurrent term [t], or [None] if
    the job hasn't been created yet (or if [t] is not an ocurrent primitive). *)
