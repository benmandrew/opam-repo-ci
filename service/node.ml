    ...
    ~ctx:
      {
        ctx with
        label =
          (let+ prefix = ctx.label and+ label in
           prefix ^ label);
      }
    ...
    ~ctx:
      {
        ...
        label =
          (let+ prefix = ctx.label and+ label in
           prefix ^ label ^ status_sep);
      }

let of_list children ~ctx =
  List.fold_left
    (fun acc t ->
      let+ acc and+ t = t ~ctx in
      ctx.merge acc t)
    (empty ~ctx) children
...
let actioned_branch (type a) ~label (action : a t) children ~ctx =
  ...
  and+ ts = dir ~label (of_list children) ~ctx in
  ctx.merge t ts

let actioned_branch_dyn ~label action children ~ctx =
  let+ t = leaf_dyn ~label action ~ctx
  and+ ts = dir_dyn ~label (of_list children) ~ctx in
  ctx.merge t ts
...
let list_map ordered ?collapse_key fn input ~ctx =
  let results =
    map_reduce ordered ?collapse_key
      ~map:(fun t -> fn t ~ctx)
      ~merge:ctx.merge ~empty:ctx.empty input
  in
  let+ state = Current.state ~hidden:true results
  and+ input = Current.state ~hidden:true input in
  ...
  | Ok _, Error (`Active _) ->
      Logs.warn (fun f ->
          f "Node.list_map: input is ready but output is pending!");
      ctx.empty
