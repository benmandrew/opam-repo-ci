...
let () =
  ...
  Printexc.register_printer (fun e ->
      Some (Printexc.to_string_default e ^ " -- " ^ Printexc.get_backtrace ()));
...
let has_role user = function
  ...
  | _ -> (
      ...
      | Some
          ...
          | "github:samoht" | "github:tmcgilchrist" | "github:benmandrew" ) ->
          true
      | _ -> false)
...
let main config mode app capnp_address github_auth submission_uri
  ...
  Lwt_main.run
    (let listen_address =
     ...
     in
     Capnp_setup.run ~listen_address capnp_address
     ...
     let routes =
       Routes.(
         ...
         @--> Current_github.webhook ~engine
                ~get_job_ids:Opam_repo_ci.Index.get_job_ids ~webhook_secret)
       :: Routes.((s "login" /? nil) @--> Current_github.Auth.login github_auth)
       :: Current_web.routes engine
     in
     let site =
       Current_web.Site.v ?authn ~has_role ~secure_cookies:true ~name:"opam-ci"
         routes
     in
     let prometheus =
