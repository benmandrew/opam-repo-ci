...
let default_compilers_full =
  Ocaml_version.Releases.[ v4_14; Ocaml_version.Releases.latest ]
(* NOTE: Should probably stay with list length 2 *)

let default_compilers =
  List.map Ocaml_version.with_just_major_and_minor default_compilers_full
...
let url ~owner ~name ~hash =
  Uri.of_string
    (Fmt.str "https://opam.ci.ocaml.org/github/%s/%s/commit/%s" owner name hash)
...
let github_status_of_state ~head result =
  ...
  let main_status =
    ...
    | _, Error (`Msg m) ->
        Github.Api.Status.v ~url `Failure ~description:("Failed - " ^ m)
  in
...
let set_active_installations installations =
  ...
  installations
  |> List.fold_left
       (fun acc i -> Index.Account_set.add (Github.Installation.account i) acc)
       Index.Account_set.empty
...
let build_spec ~platform ~opam_version pkg =
  let+ pkg in
  Build.Spec.opam ~platform ~lower_bounds:false ~with_tests:false ~opam_version
    pkg

let test_spec ~platform ~opam_version pkg =
  let+ pkg in
  Build.Spec.opam ~platform ~lower_bounds:false ~with_tests:true ~opam_version
    pkg

let lower_bounds_spec ~platform ~opam_version pkg =
  let+ pkg in
  Build.Spec.opam ~platform ~lower_bounds:true ~with_tests:false ~opam_version
    pkg

let revdep_spec ~platform ~opam_version ~revdep pkg =
  let+ revdep and+ pkg in
  Build.Spec.opam ~platform ~lower_bounds:false ~with_tests:true ~revdep
    ~opam_version pkg

(* List the revdeps of [pkg] (using [builder] and [image]) and test each one
   (using [spec] and [base], merging [source] into [master]). *)
let test_revdeps ~ocluster ~opam_version ~master ~base ~platform ~pkgopt ~after
    source =
  let revdeps =
    Build.list_revdeps ~opam_version ~base ocluster ~platform ~pkgopt ~master
      ~after source
    |> Current.map OpamPackage.Set.elements
  in
  let pkg =
    Current.map
      (fun { PackageOpt.pkg; urgent = _; has_tests = _ } -> pkg)
      pkgopt
  in
  let urgent =
    Current.map
      (fun { PackageOpt.pkg = _; urgent; has_tests = _ } -> urgent)
      pkgopt
  in
  let tests =
    revdeps
    |> Node.list_map
         ...
         (fun revdep ->
           ...
           and build = Node.action `Built image in
...
let get_significant_available_pkg = function
  | pkg, { Analyse.Analysis.kind = New; has_tests } ->
      Some { PackageOpt.pkg; urgent = None; has_tests }
  | pkg, { Analyse.Analysis.kind = SignificantlyChanged; has_tests } ->
      Some
        {
          PackageOpt.pkg;
          urgent = Some (fun (`High | `Low) -> false);
          has_tests;
        }
  | ( _,
      {
        Analyse.Analysis.kind = Deleted | Unavailable | UnsignificantlyChanged;
        _;
      } ) ->
      None
...
let build_with_cluster ~ocluster ~analysis ~lint ~master source =
  ...
  let build ~opam_version ~lower_bounds ~revdeps label variant =
    ...
    let pkgs =
      ...
      let+ _ = analysis and+ pkgs in
      pkgs
    in
    pkgs
    |> Node.list_map ~collapse_key:"pkg"
         (module PackageOpt)
         (fun pkgopt ->
           let pkg =
             Current.map
           ...
           let has_tests =
             Current.map
               (fun { PackageOpt.pkg = _; urgent = _; has_tests } -> has_tests)
               pkgopt
           in
           let base =
             ...
             | `linux ->
                 (* TODO: Use docker images as base for both macOS and linux *)
                 let+ repo_id =
                   Docker.peek ~schedule:weekly
                     ~arch:(Ocaml_version.to_docker_arch arch)
             ...
             Node.bool_map
               (fun () ->
                 let action =
                   ...
                   Build.v ocluster ~label:"test" ~base ~spec ~master ~urgent
                     source
                 in
           ...
           and lower_bounds_check =
             if lower_bounds then
               let action =
                 ...
                 Build.v ocluster ~label:"lower-bounds" ~base ~spec ~master
                   ~urgent source
               in
               let action = Node.action `Built action in
  ...
  let compilers =
    ...
    |> List.map (fun v ->
           ...
           let revdeps =
             List.exists (Ocaml_version.equal v) default_compilers
           in
           (* TODO: Remove this when the cluster is ready *)
           let v = Ocaml_version.to_string v in
           let variant =
             Variant.v ~arch:`X86_64 ~distro:master_distro ~compiler:(v, None)
           in
  ...
  let linux_distributions =
    let build ~distro ~arch ~compiler =
      ...
      let label =
        Fmt.str "%s-ocaml-%s" distro (Variant.pp_ocaml_version variant)
      in
      ...
      (fun acc comp ->
        ...
        List.fold_left
          (fun acc distro ->
            ...
            | `CentOS `V7
            | `OracleLinux _
            (* TODO: Too annoying to support. Remove when it has been removed in ocaml-dockerfile *)
            | _
              when Distro.compare distro master_distro
                   = 0 (* TODO: Add Distro.equal *)
                   || Distro.os_family_of_distro distro <> `Linux ->
                (* TODO: Unlock this when Windows is ready *)
                acc
            | _ ->
  ...
  and extras =
    ...
    let build ~opam_version ~distro ~arch ~compiler label =
      ...
      let label =
        Fmt.str "%socaml-%s" label (Variant.pp_ocaml_version variant)
      in
      build ~opam_version ~lower_bounds:false ~revdeps:false label variant
    in
    let master_distro = Distro.tag_of_distro master_distro in
      ...
      (fun acc comp_full ->
        let comp =
          Ocaml_version.to_string
            (Ocaml_version.with_just_major_and_minor comp_full)
        in
        build ~opam_version:`V2_0 ~arch:`X86_64 ~distro:master_distro
          ~compiler:(comp, None) "opam-2.0"
        :: build ~opam_version:`V2_1 ~arch:`X86_64 ~distro:master_distro
             ~compiler:(comp, None) "opam-2.1"
        :: List.filter_map
             (fun v ->
        ...
        @ List.filter_map
            (function
              ...
              | `Riscv64 ->
                  None
                  (* TODO: unlock this one when more machines are available *)
              | arch ->
                  let label = Ocaml_version.to_opam_arch arch in
                  Some
                    (build ~opam_version ~arch ~distro:master_distro
                       ~compiler:(comp, None) label))
    ...
    let main_jobs =
      if pending > 0 then Error (`Active `Running)
        ...
        | ok, err, skip ->
            Error
              (`Msg
                (Fmt.str "%d jobs failed, %d jobs skipped, %d jobs passed" err
                   skip ok))
    in
    (lint, main_jobs)
...
module Results : sig
  ...
  let merge (j1, s1) (j2, s2) =
    let impossible name _ _ =
      Printf.ksprintf failwith "Two jobs have the same name %S" name
    in
    (map2 (Index.Job_map.union impossible) j1 j2, map2 Summary.merge s1 s2)
end
...
let analyze ~master src =
  Analyse.examine ~master src |> Current.cutoff ~eq:Analyse.Analysis.equal
...
let test_pr ~ocluster ~master ~head =
  ...
  let analysis =
    latest_analysis |> latch ~label:"analysis" (* ignore errors from a rerun *)
  in
  let lint =
    let packages =
      Current.map
  ...
  let* jobs, summary =
    Node.flatten builds
      ~map:
        {
          Node.f =
            (fun ~label kind job ->
              let db_record =
                let+ variant = label and+ job_id = Node.job_id job in
      ...
      ~merge:Results.merge ~empty:Results.empty
  in
  let+ () =
    let+ jobs and+ repo and+ hash in
    Index.record ~repo ~hash jobs
  and+ summary =
    let+ summary and+ { Current_github.Repo_id.owner; name } = repo and+ hash in
...
let github_set_statuses ~head statuses =
  let+ () =
    Current.option_iter
      (Github.Api.Commit.set_status head "opam-repo-ci (linter)")
      (Current.map fst statuses)
  and+ () =
    Github.Api.Commit.set_status head "opam-ci" (Current.map snd statuses)
    (* TODO: Change to opam-repo-ci *)
  in
  ()

let test_repo ~ocluster ~push_status repo =
  let master, prs = get_prs repo in
  let master = latch ~label:"master" master in
  (* Don't cancel builds while fetching updates to this *)
  let prs = set_active_refs ~repo prs in
  prs
  |> Current.list_iter ~collapse_key:"pr" (module Github.Api.Commit)
     @@ fun head ->
     test_pr ~ocluster ~master ~head
     |> github_status_of_state ~head
     |> if push_status then github_set_statuses ~head else Current.ignore_value
...
let v ~ocluster ~app () =
  ...
  |> Current.list_iter (module Github.Installation) @@ fun installation ->
     ...
     repos
     |> Current.list_iter (module Github.Api.Repo) @@ fun repo ->
        Current.all
          [
