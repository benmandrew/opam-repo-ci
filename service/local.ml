...
let opam_repository =
  { Github.Repo_id.owner = "ocaml"; name = "opam-repository" }
...
let main config mode capnp_address submission_uri api repo_id prometheus_config
  ...
  Lwt_main.run
    ( Capnp_setup.run capnp_address >>= fun (vat, rpc_engine_resolver) ->
      ...
      let engine =
        Current.Engine.create ~config (Pipeline.local_test ~ocluster repo)
      in
      rpc_engine_resolver
      |> Option.iter (fun r ->
      ...
      let site =
        Current_web.Site.(v ~has_role:allow_all)
          ~name:"opam-repo-ci-local" routes
      in
      let prometheus =
        List.map (Lwt.map @@ Result.ok)
