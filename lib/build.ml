...
module Spec = struct
  ...
  type list_revdeps = { opam_version : [ `V2_0 | `V2_1 | `Dev ] }
  [@@deriving to_yojson]

  type ty =
    [ `Opam of
      [ `Build of opam_build | `List_revdeps of list_revdeps ] * package ]
  [@@deriving to_yojson]

  type t = { platform : Platform.t; ty : ty }
...
type t = { connection : Current_ocluster.Connection.t; timeout : Duration.t }
...
module Op = struct
  ...
  module Key = struct
    type t = {
      ...
      variant : Variant.t;
          (* Added as a comment in the Dockerfile and selects personality *)
      ty : Spec.ty;
    }
  ...
  let run { config = { connection; timeout }; master; urgent; base } job
      { Key.pool; commit; variant; ty } () =
    let master = Current_git.Commit.hash master in
    let os =
      match Variant.os variant with `macOS | `linux | `FreeBSD -> `Unix
    in
    let build_spec ~for_docker =
      ...
      | `Opam (`Build { revdep; lower_bounds; with_tests; opam_version }, pkg)
        ->
          Opam_build.spec ~for_docker ~opam_version ~base ~variant ~revdep
            ~lower_bounds ~with_tests ~pkg
    in
    Current.Job.write job
      (Fmt.str
         "@.To reproduce locally:@.@.cd $(mktemp -d)@.%a@.git fetch origin \
          master@.git merge --no-edit %s@.cat > ../Dockerfile \
          <<'END-OF-DOCKERFILE'@.\o033[34m%s\o033[0m@.END-OF-DOCKERFILE@.docker \
          build -f ../Dockerfile .@.@."
         Current_git.Commit_id.pp_user_clone commit master
         (Obuilder_spec.Docker.dockerfile_of_spec ~os ~buildkit:false
            (build_spec ~for_docker:true)));
    let spec_str =
      Fmt.to_to_string Obuilder_spec.pp (build_spec ~for_docker:false)
    in
    let action = Cluster_api.Submission.obuilder_build spec_str in
    let src =
      (Git.Commit_id.repo commit, [ master; Git.Commit_id.hash commit ])
    in
    let cache_hint =
      let pkg =
        match ty with
        | `Opam (`Build { revdep = Some revdep; _ }, pkg) ->
            Fmt.str "%s-%s"
              (OpamPackage.to_string pkg)
              (OpamPackage.to_string revdep)
        | `Opam (`List_revdeps _, pkg) | `Opam (`Build _, pkg) ->
            OpamPackage.to_string pkg
      in
    ...
    let build_pool =
      Current_ocluster.Connection.pool ?urgent ~job ~pool ~action ~cache_hint
        ~src connection
    in
    ...
    let buffer =
      ...
      | _ -> None
    in
    Capability.with_ref build_job (run_job ?buffer ~job)
    >>!= fun (_ : string) ->
    match buffer with
  ...
  let pp f ({ Key.pool = _; commit; variant; ty }, _) =
    Fmt.pf f "@[<v>%a@,from %a@,on %a@]" Spec.pp_ty ty Current_git.Commit_id.pp
      commit Variant.pp variant
...
(* TODO: Create a separate type of cache that doesn't parse the output *)
            ...
            |> List.fold_left
                 (fun acc -> function
                   ...
                   | revdep ->
                       ...
                       if OpamPackage.equal pkg revdep then acc
                         (* NOTE: opam list --recursive --depends-on <pkg> also returns <pkg> itself *)
                       else OpamPackage.Set.add revdep acc)
