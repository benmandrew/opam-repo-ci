...
val record : repo:Current_github.Repo_id.t -> hash:string -> job_ids -> unit
(** [record ~repo ~hash jobs] updates the entry for [repo, hash] to point at
    [jobs]. *)

val is_known_repo : owner:string -> name:string -> bool
(** [is_known_repo ~owner ~name] is [true] iff there is an entry for a commit in
...
val get_build_status :
  owner:string -> name:string -> hash:string -> build_status
(** [get_build_status ~owner ~name ~hash] is the latest status for this
    combination. *)

val get_n_jobs : owner:string -> name:string -> hash:string -> int
(** [get_n_jobs ~owner ~name ~hash] is the latest number of jobs for this
    combination. *)

val set_status :
  owner:string -> name:string -> hash:string -> build_status * int -> unit
(** [set_status ~owner ~name ~hash build_status] sets the latest status and
    number of jobs for this combination. *)
...
(** [set_active_accounts accounts] record that [accounts] is the set of accounts
    on which the CI is installed. This is displayed in the CI web interface. *)

val get_active_accounts : unit -> Account_set.t
(** [get_active_accounts ()] is the last value passed to [set_active_accounts],
    or [[]] if not known yet. *)

val set_active_refs :
  repo:Current_github.Repo_id.t -> (string * string) list -> unit
(** [set_active_refs ~repo entries] records that [entries] is the current set of
    Git references that the CI is watching. There is one entry for each branch
    and PR. Each entry is a pair of the Git reference name and the head commit's
    hash. *)
...
(** [get_active_refs repo] is the entries last set for [repo] with
    [set_active_refs], or [] if this repository isn't known. *)
