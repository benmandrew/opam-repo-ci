...
CREATE TABLE IF NOT EXISTS ci_build_index (
  ...
  PRIMARY KEY (owner, name, hash, variant)
     ...
     let list_repos =
       Sqlite3.prepare db
         "SELECT DISTINCT name FROM ci_build_index WHERE owner = ?"
     in
     let repo_exists =
       Sqlite3.prepare db
         "SELECT EXISTS (SELECT 1 FROM ci_build_index WHERE owner = ? AND name \
          = ?)"
     in
     let get_jobs =
       Sqlite3.prepare db
...
let get_job_ids_with_variant t ~owner ~name ~hash =
  ...
  |> List.fold_left
       (fun acc -> function
         ...
         | Sqlite3.Data.[ TEXT variant; TEXT id ] ->
             Job_map.add variant (Some id) acc
...
module Metrics = struct
  let empty_n_per_status =
    { not_started = 0; pending = 0; failed = 0; passed = 0 }
...
module Commit_info_cache = struct
  ...
  type elt = { build_status : build_status; n_jobs : int }
  ...
  let add ~owner ~name ~hash status =
    if Hashtbl.length cache > cache_max_size then (
      Hashtbl.clear cache;
    ...
    |> Option.iter (fun x ->
           Metrics.modify_n_per_status (fun x -> x - 1) x.build_status);
  ...
  let find_build_status ~owner ~name ~hash =
    (find ~owner ~name ~hash).build_status
...
let set_status ~owner ~name ~hash (build_status, n_jobs) =
  Commit_info_cache.add ~owner ~name ~hash
    Commit_info_cache.{ build_status; n_jobs }
...
let set_active_refs ~repo (refs : (string * string) list) =
  ...
  |> Option.iter (fun old_refs ->
         ...
         let removed_refs =
           List.filter (fun k -> not @@ List.mem k refs) old_refs
         in
         Metrics.inc_n_handled (List.length removed_refs);
         List.iter
           (fun (_, hash) ->
             Commit_info_cache.remove ~owner:repo.Current_github.Repo_id.owner
               ~name:repo.name ~hash)
           removed_refs);
...
let get_jobs_per_ref repo =
  ...
  Commit_info_cache.get_jobs_per_ref ~owner:repo.Current_github.Repo_id.owner
    ~name:repo.name active_refs
