...
module OpamPackage = struct
  ...
  let of_yojson x =
    Result.map OpamPackage.of_string ([%derive.of_yojson: string] x)
end
...
module Analysis = struct
  ...
  type data = { kind : kind; has_tests : bool } [@@deriving eq, yojson]
  type t = { packages : (OpamPackage.t * data) list } [@@deriving eq, yojson]
  ...
  let check_opam opam =
    ...
    (match OpamFile.OPAM.format_errors opam with
    | [] -> ()
    | errors ->
        let errors =
          errors
          |> List.map (fun (_, err) ->
                 "  - " ^ OpamPp.string_of_bad_format (OpamPp.Bad_format err))
          |> String.concat "\n"
        in
        Fmt.failwith "Format errors detected in %s:\n%s"
          (OpamPackage.to_string pkg)
          errors);
    match OpamFile.OPAM.available opam with OpamTypes.FBool b -> b | _ -> true

  let get_package_name ~path ~name ~package =
    let nme =
      try OpamPackage.Name.of_string name
      with Failure msg ->
        Fmt.failwith "%S is not a valid package name (in %S): %s" name path msg
    in
    let pkg =
      try OpamPackage.of_string package
      with Failure msg ->
        Fmt.failwith "%S is not a valid package name.version (in %S): %s"
          package path msg
    in
    if OpamPackage.Name.compare nme (OpamPackage.name pkg) <> 0 then
      Fmt.failwith
        "Mismatch between package dir name %S and parent directory name %S"
        package name;
    pkg
  ...
  let ci_extensions_equal old_file new_file =
    let filter_ci_exts =
      OpamStd.String.Map.filter (fun name _ ->
          OpamStd.String.starts_with ~prefix:"x-ci-" name)
    in
  ...
  let add_pkg ~path ~name ~package kind pkgs =
    let update old_kind =
      match (old_kind, kind) with
      (* NOTE: Impossible combinations (opam file would have to be processed more than once) *)
      | (New | Deleted | Unavailable), (New | Deleted | Unavailable) -> old_kind
      (* old_kind instead of assert false because OpamStd.Map.update works this way :( *)
      ...
      | ( UnsignificantlyChanged,
          (New | Deleted | Unavailable | SignificantlyChanged) ) ->
          kind
    in
    OpamPackage.Map.update
      (get_package_name ~path ~name ~package)
      update kind pkgs
  ...
  let get_opam ~cwd path =
    Lwt.catch
      (fun () ->
        ...
        >>= Lwt.return_ok)
      (function Unix.Unix_error _ -> Lwt.return (Error ()) | e -> Lwt.fail e)
         ...
         (fun pkgs path ->
           ...
           | path -> (
               ...
               | [ "packages"; name; package; "opam" ] -> (
                   ...
                   Current.Process.check_output ~cwd:dir ~cancellable:true ~job
                     cmd
                   >>= function
                   | Error _ ->
                       (* new release *)
                   ...
                   | Ok old_content ->
                       ...
                       >>= Lwt_preemptive.detach (function
                             | Error () ->
                                 (* deleted package *)
                             ...
                             | Ok new_content ->
                                 (* modified package *)
                                 let filename =
                                   OpamFile.make (OpamFilename.raw path)
                                 in
                                 let old_file =
                                   try
                                     OpamFile.OPAM.read_from_string ~filename
                                       old_content
                                   with
                                   | OpamPp.Bad_format _ | OpamPp.Bad_version _
                                   ->
                                     OpamFile.OPAM.empty
                                 in
                                 let new_file =
                                   ...
                                   ->
                                     Fmt.failwith "%S failed to be parsed: %s"
                                       path msg
                                 in
                                 if not (check_opam new_file) then
                                   ...
                                   OpamFile.OPAM.effectively_equal old_file
                                     new_file
                                 ...
                                 then
                                   ...
                                   add_pkg ~path ~name ~package
                                     UnsignificantlyChanged pkgs
                                 else
                                   add_pkg ~path ~name ~package
                                     SignificantlyChanged pkgs))
               | _ ->
                   Fmt.failwith
                     "Unexpected path %S in output (expecting \
                      'packages/name/pkg/...')"
                     path))
    ...
    let has_with_test_variable () =
      ...
      let aux variable =
        if OpamVariable.Full.is_global variable then
          match
            OpamVariable.to_string (OpamVariable.Full.variable variable)
          with
      ...
      in
      try
        let _ : OpamFile.OPAM.t = OpamFileTools.map_all_variables aux opam in
        false
...
module Examine = struct
  ...
  module Key = struct
    type t = { src : Current_git.Commit.t }

    let digest { src } = Current_git.Commit.hash src
  end
  ...
  module Value = struct
    ...
    let digest { master } =
      let json =
        `Assoc [ ("master", `String (Current_git.Commit.hash master)) ]
      in
...
let examine ~master src =
  ...
  |>
  let> src and> master in
  Examine_cache.run Examine.No_context { Examine.Key.src }
    { Examine.Value.master }
