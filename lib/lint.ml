...
let () = Lwt_preemptive.init 0 1 (fun _errlog -> ())

(* NOTE: Lwt_preemptive is used to wrap
...
module Check = struct
  ...
  let get_opam ~cwd pkg =
    Analyse.Analysis.get_opam ~cwd (path_from_pkg pkg // "opam")
    >>/= fun opam -> Lwt.return (OpamFile.OPAM.read_from_string opam)
    ...
    let rec aux files =
      Lwt.catch
        (fun () ->
          ...
          if Fpath.is_rel_seg file then aux files else aux (file :: files))
        (function End_of_file -> Lwt.return files | exn -> Lwt.fail exn)
    in
    Lwt.finalize (fun () -> aux []) (fun () -> Lwt_unix.closedir dir)
  ...
  let scan_dir ~cwd errors pkg =
    ...
    let rec aux errors extra_files = function
      ...
      | "opam" :: files ->
          (is_perm_644 (dir // "opam") >|= function
           | true -> errors
           | false -> (pkg, ForbiddenPerm (dir // "opam")) :: errors)
          >>= fun errors -> aux errors extra_files files
      | "files" :: files ->
          ...
          Lwt_list.fold_left_s
            ...
            errors extra_files
          >>= fun errors ->
          let check_hash file hash =
            try OpamHash.check_file file hash with _ -> false
          in
          let extra_files =
            List.map
              (fun file ->
                ( OpamFilename.Base.of_string file,
                  check_hash (dir // "files" // file) ))
              extra_files
          in
  ...
  let get_dune_project_version ~pkg url =
    ...
    Lwt_preemptive.detach
      ...
      ()
    >>= function
    | OpamTypes.Not_available (_, msg) -> Lwt.return (Error msg)
    | Up_to_date _ | Result _ ->
        let dune_project = Filename.concat dir "dune-project" in
        Lwt.catch
          (fun () ->
            Lwt_io.with_file ~mode:Lwt_io.Input dune_project (fun ch ->
                Lwt_io.read ch >|= Sexplib.Sexp.parse)
            >>= function
            | Sexplib.Sexp.(
                Done (List [ Atom "lang"; Atom "dune"; Atom version ], _)) ->
                Lwt.return (Ok (Some version))
            | Done _ ->
                Lwt.fail_with "(lang dune ...) is not the first construct"
  ...
  let check_dune_constraints ~errors ~pkg opam =
    ...
    | Some url ->
        ...
        let errors =
          ...
          | None, Ok (Some _) ->
              if is_dune (OpamPackage.name pkg) then errors
              else (pkg, DuneConstraintMissing) :: errors
          | Some dep, Ok (Some ver) ->
              if OpamVersionCompare.compare dep ver >= 0 then errors
              else (pkg, BadDuneConstraint (dep, ver)) :: errors
        in
        if is_build then (pkg, DuneIsBuild) :: errors else errors
    | None -> Lwt.return errors
  ...
  let of_dir ~master ~job ~packages cwd =
    ...
    Lwt_list.fold_left_s
      (fun errors (pkg, kind) ->
        match kind with
        | Analyse.Analysis.Deleted -> Lwt.return errors (* TODO *)
        | Analyse.Analysis.(
            New | Unavailable | SignificantlyChanged | UnsignificantlyChanged)
          ->
            ...
            let errors =
              ...
              | Some name ->
                  ...
                  else (pkg, UnmatchedName name) :: errors
            in
            (* Check version field *)
            let errors =
              ...
              | None -> errors
              | Some version ->
                  if OpamPackage.Version.equal version (OpamPackage.version pkg)
                  then (pkg, UnnecessaryField "version") :: errors
                  else (pkg, UnmatchedVersion version) :: errors
            in
            ...
            let errors =
              List.fold_left
                (fun errors -> function
                  | OpamTypes.(
                      ( [ (CString "dune", None); (CString "subst", None) ],
                        filter )) -> (
                      match filter with
                  ...
                  | _ -> errors)
                errors opam.OpamFile.OPAM.build
            in
              ...
              (fun () ->
                ...
                |> List.fold_left
                     (fun errors -> function
                       | 67, _, _ ->
                           errors
                           (* TODO: Disable error 67 (Checksum specified with a non archive url) while discussing a fix:
                              - https://github.com/ocaml/opam/pull/4834
                              - https://github.com/ocaml/opam/pull/4960 *)
                       | x -> (pkg, OpamLint x) :: errors)
                     errors)
              ()
            >>= fun errors -> Lwt.return errors)
      [] packages
end

module Lint = struct
  type t = { master : Current_git.Commit.t }
  ...
  let msg_of_errors =
    List.map (fun (package, err) ->
        ...
        | UnnecessaryField field ->
            Fmt.str
              "Warning in %s: Unnecessary field '%s'. It is suggested to \
               remove it."
              pkg field
        | UnmatchedName value ->
            Fmt.str
              "Error in %s: The field 'name' that doesn't match its context. \
        ...
        | UnmatchedVersion value ->
            Fmt.str
              "Error in %s: The field 'version' that doesn't match its \
               context. Field 'version' has value '%s' but was expected of \
               value '%s'."
              pkg
        ...
        | DubiousDuneSubst ->
            Fmt.str
              "Warning in %s: Dubious use of 'dune subst'. 'dune subst' should \
               always only be called with {dev} (i.e. [\"dune\" \"subst\"] \
               {dev}) If your opam file has been autogenerated by dune, you \
               need to upgrade your dune-project to at least (lang dune 2.7)."
              pkg
        | DuneProjectMissing ->
            Fmt.str
              "Warning in %s: The package seems to use dune but the \
               dune-project file is missing."
              pkg
        | DuneConstraintMissing ->
            Fmt.str
              "Warning in %s: The package has a dune-project file but no \
               explicit dependency on dune was found."
              pkg
        | DuneIsBuild ->
            Fmt.str
              "Warning in %s: The package tagged dune as a build dependency. \
               Due to a bug in dune \
               (https://github.com/ocaml/dune/issues/2147) this should never \
               be the case. Please remove the {build} tag from its filter."
              pkg
        | BadDuneConstraint (dep, ver) ->
            Fmt.str
              "Error in %s: Your dune-project file indicates that this package \
               ...
               >= %s. Please check which requirement is the right one, and fix \
               the other."
              pkg ver dep
        | UnexpectedFile file ->
            Fmt.str "Error in %s: Unexpected file in %s/files/%s" pkg
              (Check.path_from_pkg package)
              file
        | ForbiddenPerm file ->
            Fmt.str
              "Error in %s: Forbidden permission for file %s/%s. All files \
        ...
        | FailedToDownload msg ->
            Fmt.str "Error in %s: Failed to download the archive. Details: %s"
              pkg msg)
...
let check ~master ~packages src =
  ...
  |>
  let> src and> packages and> master in
