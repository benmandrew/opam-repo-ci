...
let opam_install ~variant ~opam_version ~pin ~lower_bounds ~with_tests ~pkg =
  ...
  (if lower_bounds then
     ...
     [
       ...
       env "OPAMCRITERIA"
         "-removed,-count[avoid-version,changed],-count[version-lag,request],-count[version-lag,changed],-count[missing-depexts,changed],-changed";
       env "OPAMFIXUPCRITERIA"
         "-removed,-count[avoid-version,changed],-count[version-lag,request],-count[version-lag,changed],-count[missing-depexts,changed],-changed";
       env "OPAMUPGRADECRITERIA"
         "-removed,-count[avoid-version,changed],-count[version-lag,request],-count[version-lag,changed],-count[missing-depexts,changed],-changed";
     ])
  @ (if pin then
       let version =
  ...
  @ [
      ...
      (let depext =
       ...
       in
       run
         ~cache:(if with_tests then [] else cache)
         ~network:(if with_tests then [] else network)
...
let setup_repository ~variant ~for_docker ~opam_version =
  ...
  in
  let opam_version_str =
    match opam_version with `V2_0 -> "2.0" | `V2_1 -> "2.1" | `Dev -> "dev"
  in
  let opam_repo_args =
    match Variant.os variant with
    | `macOS ->
        " -k local"
        (* TODO: (copy ...) do not copy the content of .git or something like that and make the subsequent opam pin fail *)
    | `FreeBSD | `linux -> ""
  in
  let opamrc =
    match Variant.os variant with
  ...
  :: (* Advertise CI for test frameworks *)
     [
       ...
       run "opam repository set-url%s --strict default opam-repository/"
         opam_repo_args;
       run ~network "opam %s || true"
         (match opam_version with
         | `V2_1 | `Dev -> "update --depexts"
         | `V2_0 -> "depext -u");
     ]
...
let spec ~for_docker ~opam_version ~base ~variant ~revdep ~lower_bounds
    ~with_tests ~pkg =
  ...
  let revdep =
    ...
    | None -> []
    | Some revdep ->
        opam_install ~pin:false ~lower_bounds:false ~with_tests:false
          ~pkg:revdep
  and tests =
    match (with_tests, revdep) with
    | true, None ->
        opam_install ~pin:false ~lower_bounds:false ~with_tests:true ~pkg
    | true, Some revdep ->
        opam_install ~pin:false ~lower_bounds:false ~with_tests:true ~pkg:revdep
    | false, _ -> []
  and lower_bounds =
...
let revdeps ~for_docker ~opam_version ~base ~variant ~pkg =
  ...
  Obuilder_spec.stage ~from:base
    ...
    @ [
        run
          "echo '@@@OUTPUT' && opam list -s --color=never --depends-on %s \
           --coinstallable-with %s --installable --all-versions --depopts && \
           opam list -s --color=never --depends-on %s --coinstallable-with %s \
           --installable --all-versions --recursive && opam list -s \
           --color=never --depends-on %s --coinstallable-with %s --installable \
