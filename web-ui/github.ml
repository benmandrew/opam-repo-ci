...
let normal_response x = x >|= fun x -> `Response x
...
let org_url owner = Printf.sprintf "/github/%s" owner
let repo_url ~owner name = Printf.sprintf "/github/%s/%s" owner name
...
module StatusTree : sig
  ...
  type 'a tree = Leaf of key * 'a | Branch of key * 'a option * 'a t
  and 'a t = 'a tree list
  ...
  let rec add k x ts =
    ...
    | [ k ], Branch (k', None, t) :: ts when String.equal k k' ->
        Branch (k, Some x, t) :: ts
    | [ k ], (Branch _ as t) :: ts -> t :: add [ k ] x ts
    | k :: ks, [] -> [ Branch (k, None, add ks x []) ]
    | k :: ks, Leaf (k', y) :: ts when String.equal k k' ->
        Branch (k, Some y, add ks x []) :: ts
    | k :: ks, Branch (k', y, t) :: ts when String.equal k k' ->
        Branch (k, y, add ks x t) :: ts
...
let statuses ss =
  ...
  let rec render_status : (Client.State.t * _) StatusTree.tree -> _ = function
    | StatusTree.Leaf (_, x) -> status x []
    | StatusTree.Branch (b, None, ss) ->
        (* TODO: Remove that *)
        let b =
          if
            Astring.String.is_prefix ~affix:"macos-homebrew" b
    ...
    | StatusTree.Branch
        (_, Some (((NotStarted | Aborted | Failed _ | Undefined _), _) as x), _)
      ->
        status x []
        (* Do not show children of a node that has failed (guarenties in service/pipeline.ml means that only successful parents have children with meaningful error messages) *)
...
let format_refs ~owner ~name refs =
  let open Tyxml.Html in
  ul
    (Client.Ref_map.to_seq refs
    |> List.of_seq
    |> List.map @@ fun (branch, commit) ->
                 ...
                 | "refs" :: "heads" :: branch ->
                     ...
                     span
                       [
                         txt "branch ";
                         a
                           ~a:[ a_href (github_branch_url ~owner ~name branch) ]
                           [ txt branch ];
                       ]
                 | [ "refs"; "pull"; id; "head" ] ->
                     span
                       [
                         txt "PR ";
                         a
                           ~a:[ a_href (github_pr_url ~owner ~name id) ]
                           [ txt ("#" ^ id) ];
                       ]
...
let link_jobs ~owner ~name ~hash ?selected jobs =
  ...
  in
  error_tree @ [ details (summary [ b [ txt "Full results" ] ]) [ full_tree ] ]

let short_hash = Astring.String.with_range ~len:6

let stream_logs job ~owner ~name ~refs ~hash ~variant ~status (data, next)
    writer =
  let header, footer =
  ...
  Transfer_IO.write writer (header ^ Current_ansi.process ansi data)
  >>= fun () ->
  let rec aux next =
    Current_rpc.Job.log job ~start:next >>= function
    | Ok ("", _) -> Transfer_IO.write writer footer
    | Ok (data, next) ->
        Transfer_IO.write writer (Current_ansi.process ansi data) >>= fun () ->
        aux next
    | Error (`Capnp ex) ->
        Log.warn (fun f -> f "Error fetching logs: %a" Capnp_rpc.Error.pp ex);
        Transfer_IO.write writer
          (Fmt.str "opam-repo-ci error: %a@." Capnp_rpc.Error.pp ex)
  in
  aux next
...
let repo_handle ~meth ~owner ~name ~repo path =
  ...
  | `GET, [] ->
      ...
      let body =
        Template.instance
          [
            breadcrumbs [ ("github", "github"); (owner, owner) ] name;
            format_refs ~owner ~name refs;
          ]
      in
      Server.respond_string ~status:`OK ~body () |> normal_response
  | `GET, [ "commit"; hash ] ->
      Capability.with_ref (Client.Repo.commit_of_hash repo hash)
      @@ fun commit ->
  ...
  | `GET, [ "commit"; hash; "variant"; variant ] ->
      ...
      in
      let res =
        Cohttp.Response.make ~status:`OK ~flush:true
          ~encoding:Cohttp.Transfer.Chunked ~headers ()
      in
      let write _ic oc =
        let flush = Cohttp.Response.flush res in
        let writer =
          Transfer_IO.make_writer ~flush Cohttp.Transfer.Chunked oc
        in
        Lwt.finalize
          (fun () ->
            stream_logs job ~owner ~name ~refs ~hash ~variant ~status chunk
              writer
            >>= fun () -> Server.IO.write oc "0\r\n\r\n")
          (fun () ->
  ...
  | `POST, [ "commit"; hash; ("rebuild-failed" as rebuild_mode) ] ->
      ...
      let can_rebuild (commit : Client.Commit.t) (job_i : Client.job_info) =
        if
          rebuild_all
          && not (is_a_job_triggering_other_jobs job_i.Client.variant)
          || can_rebuild job_i
        then
          let variant = job_i.variant in
          Capability.with_ref (Client.Commit.job_of_variant commit variant)
          @@ fun job -> Lwt.return (Some (job_i, job))
        else Lwt.return None
      in
      let rebuild_many (commit : Client.Commit.t)
          (job_infos : Client.job_info list) =
        Lwt_list.fold_left_s
          (fun (success, failed) job_info ->
            can_rebuild commit job_info >>= function
            | None -> Lwt.return (success, failed)
            | Some (ji, j) -> (
                Capability.with_ref (Current_rpc.Job.rebuild j)
                @@ fun new_job ->
                Capability.await_settled new_job >|= function
              ...
              [
                span
                  [
                    txt
                    @@ Fmt.str
                         "%d job%s could not be rebuilt. Check logs for more \
                          detail."
                         n
                         (if n >= 1 then "s" else "");
                  ];
        ...
        Lwt_list.fold_left_s
          (fun (success, failed) job_info ->
            ...
            | Some (ji, j) -> (
                ...
                | Error (`Capnp ex) ->
                    Log.err (fun f ->
                        f "Error cancelling job: %a" Capnp_rpc.Error.pp ex);
                    (success, succ failed)))
          ([], 0) job_infos
      in
      Capability.with_ref (Client.Repo.commit_of_hash repo hash)
              ...
              [
                span
                  [
                    txt
                    @@ Fmt.str
                         "%d job%s could not be cancelled. Check logs for more \
                          detail."
                         n
                         (if n >= 1 then "s" else "");
                  ];
