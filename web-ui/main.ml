...
let handle_request ~backend _conn request _body =
  ...
  | `GET, [ "css"; "style.css" ] -> Style.get () |> normal_response
  | meth, "github" :: path -> Github.handle ~backend ~meth path
  | `GET, "badge" :: path -> Badges.handle ~backend ~path
...
let main port backend_uri prometheus_config =
  Lwt_main.run
    (let vat = Capnp_rpc_unix.client_only_vat () in
     ...
     let config =
       Server.make_response_action ~callback:(handle_request ~backend) ()
     in
       ...
       Lwt.try_bind
         ...
         (function
           | Unix.Unix_error (Unix.EADDRINUSE, "bind", _) ->
               Fmt.failwith
                 "Web-server failed.@ Another program is already using this \
                  port %a."
                 pp_mode mode
           | ex -> Lwt.fail ex)
     in
